...
<script type="text/javascript">
	function list() {
		document.listFrm.action = "admin.jsp";
	...
	function read(idx) {
		...
		document.readFrm.action = "http://localhost:8080/Web/board/read.jsp";
		document.readFrm.submit();
	}
	function del(idx) {
		document.readFrm.idx.value = idx;
		document.readFrm.action = "http://localhost:8080/Web/board/adminDelete.jsp";
		document.readFrm.submit();
	}
	function check() {
...
<body>
		...
		<div class="page-wrap">
			<div class="content">
				...
				<br /> <a href="http://localhost:8080/Web/main.jsp?pagefile=board/post">[글쓰기]</a>
				<%
				try {
				String id = (String) session.getAttribute("id");
					if (id.equals("admin")) {  %>
				 <a href="http://localhost:8080/Web/main.jsp?pagefile=board/list">[돌아가기]</a>
				<%
					}
				} catch (Exception e) {
						e.printStackTrace();
 ...
 %></a></li>
				...
				<!-- 페이징 및 블럭 처리 End-->
				<form name="searchFrm" method="post" action="board/list.jsp">
					<div class="boardSearch">
						<select name="keyField">
							<option value="title" selected="selected">제목</option>
							<option value="mem_name">글쓴이</option>
							<option value="content">내용</option>
						</select> <input type="text" id="txt" name="keyWord" /> 
